---
name: critique-bible
description: Adversarial structural review of `output/<series>/bible.md` — the world bible. Audits the foundational worldbuilding document BEFORE `book-setup` or `plan-book` run, so that the per-book plan never inherits a flawed foundation. Use this when the user says "audita la biblia" / "critique the bible" / "is the bible ready?" Invoke after writing or revising the bible, and before running `book-setup` for a new book in the series.
---

# critique-bible

You are running the **critique-bible** skill. The bible is the
foundational worldbuilding document for the whole series. Everything
downstream (`setup.md`, `canon/*`, `plan/*`, chapters) ultimately
quotes from it. If the bible has a structural gap, every per-book
plan inherits it — and the writer will discover it mid-prose.

This skill exists so that **the bible is bulletproof before any per-book
...
- The author has finished a draft of `output/<series>/bible.md` (or
...
- The author says "audita la biblia" / "critique the bible" / "está
  lista la biblia para empezar?"

## Hard rules

- **Adversarial bias is total.** A bible that passes with zero findings
...
- **Do not rewrite the bible.** Quote, name, point. Concrete direction
  yes; substitute text no. The author writes the bible.
...
```bash
python3 .claude/skills/critique-bible/scripts/audit_bible.py \
    --series-slug <slug> \
    --output output/<slug>/notes/_audit-bible.md
```
...
If `bible.md` does not exist, the audit refuses and tells the user to
start from `references/bible-template.md`.

### 2. Read the bible in full

Read `output/<series>/bible.md` and the audit report. Also read these
...
- ≥ 2 principals with full arcs declared?
- Do their wounds/wants/lies differ? If two principals carry the same
  lie or the same decision-shape, the book has one arc, not two.
- Does the antagonist have a thesis — a worldview that, in different
  light, would be correct? If they're stock evil, that's MUST fix.
- Is there a deuteragonist or secondary POV beyond the protagonist
  and antagonist? Epic fantasy without one feels claustrophobic.
...
#### 3k. Cross-book inheritance (trilogy only)
...
- What dies between books (characters, factions, places)? Is it
  named?
- What promises does the bible make in Book I that Book III must
  pay?
...
```
output/<series>/notes/critique-bible.md
```

Structure:

```markdown
# Bible critique — <series>
...
## What works
- 4-6 brief notes on what the bible does well. The author needs the
...
- If a per-book plan already exists (`book-NN/`), note any
  inconsistencies between bible and book setup.
```

Thresholds:
- Any `MUST fix` → **REJECT**. The bible is not ready for `book-setup`.
...
- Suggested next step:
  - **REJECT** → revise bible, then re-run `critique-bible`.
  - **REVISE** → walk through SHOULDs interactively; author updates
    bible; re-run.
  - **PASS** → `book-setup` for Book I (or for the next book in
    series), with bible as fixed reference.

### 6. Compose with `critique-plan`

If a per-book plan already exists for Book N (`output/<series>/book-NN/`),
`critique-plan` should be re-run AFTER `critique-bible` PASS. The
per-book audit can then assume the bible is sound and focus on the
book-level plan against the bible's contract.

## What this skill does NOT do

- Does not modify `bible.md`. The author writes the bible.
...
- Does not auto-parse the bible into `setup.md` (no `ingest-bible` —
  intentional design decision).

## Files this skill writes

- `output/<series>/notes/_audit-bible.md` — deterministic audit.
- `output/<series>/notes/critique-bible.md` — qualitative critique.
