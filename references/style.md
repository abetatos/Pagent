# House style — master template

> This is the **master style template**. It is the positive companion to
> `prose-antipatterns.md`: that file lists what to *avoid*; this one
> declares how prose should *sound* and feel.
>
> It is **not read directly when writing**. Instead, `book-setup` copies
> everything from the first `##` heading below into each new book's own
> `output/<series>/book-NN/style.md`. That per-book copy is the single
> source of truth for that book's style — edit it freely, per book.
> Editing this template changes only what *future* books start from.
